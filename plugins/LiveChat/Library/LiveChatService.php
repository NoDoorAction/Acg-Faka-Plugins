...
class LiveChatService
{
    ...
    public static function ensureSchema(): void
    {
        ...
        $prefix = preg_replace('/[^A-Za-z0-9_]/', '', (string)($database['prefix'] ?? ''));
        $sessionTable = "`{$prefix}plugin_livechat_session`";
        $messageTable = "`{$prefix}plugin_livechat_message`";
        ...
        try {
            ...
            throw new JSONException('LiveChat 数据表初始化失败，请检查数据库权限或手动执行 install.sql');
        }
    ...
    public static function closeSession(int $sessionId, string $closedBy): void
    {
        ...
        DB::transaction(function () use ($sessionId, $closedBy, $now) {
            DB::table('plugin_livechat_message')->where('session_id', $sessionId)->delete();
            DB::table('plugin_livechat_session')->where('id', $sessionId)->update([

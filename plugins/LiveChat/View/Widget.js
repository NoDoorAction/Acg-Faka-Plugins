(function () {
    ...
    const api = {
        bootstrap: "/plugin/LiveChat/api/bootstrap",
        send: "/plugin/LiveChat/api/send",
        poll: "/plugin/LiveChat/api/poll",
        end: "/plugin/LiveChat/api/end"
    };
    ...
    const parseJsonResponse = async response => {
        ...
        if (!contentType.includes("application/json")) {
            ...
            if (text.includes("404 Not Found")) {
                throw new Error("接口返回404，请确认LiveChat文件已更新并且数据库表已创建");
            }
            throw new Error("接口返回非JSON响应，请检查服务器错误日志");
        }
    ...
    root.innerHTML = `
        ...
        <section class="lc-widget ${state.open ? "" : "lc-hidden"}" aria-live="polite">
            ...
            <main class="lc-view lc-view-chat lc-hidden">
                ...
                <div class="lc-ended lc-hidden">会话已结束，聊天内容已清理。</div>
    ...
    const endSession = () => {
        ...
        if (!window.confirm("确定结束当前会话吗？聊天内容会被清理。")) return;
